...
install_dependencies() {
    ...
    if command -v pkg >/dev/null 2>&1; then
        ...
        if ! command -v git >/dev/null 2>&1; then
            echo "Installing Git..."
            pkg install git -y || { echo "Error: Failed to install Git."; exit 1; }
        else
            echo "Git is already installed."
        fi
        if ! command -v curl >/dev/null 2>&1; then
            echo "Installing curl..."
            pkg install curl -y || { echo "Error: Failed to install curl."; exit 1; }
        else
        ...
        if ! command -v git >/dev/null 2>&1; then
            echo "Installing Git..."
            sudo apt install git -y || { echo "Error: Failed to install Git."; exit 1; }
        else
            echo "Git is already installed."
        fi
        if ! command -v curl >/dev/null 2>&1; then
            echo "Installing curl..."
            sudo apt install curl -y || { echo "Error: Failed to install curl."; exit 1; }
        else
            echo "curl is already installed."
        fi
        ...
        if ! command -v git >/dev/null 2>&1; then
            echo "Installing Git..."
            brew install git || { echo "Error: Failed to install Git."; exit 1; }
        else
            echo "Git is already installed."
        fi
        if ! command -v curl >/dev/null 2>&1; then
            echo "Installing curl..."
            brew install curl || { echo "Error: Failed to install curl."; exit 1; }
        else
            echo "curl is already installed."
        fi
    else
...
delete_script() {
    ...
    echo "Note: This option is not applicable when running via curl."
    echo "Please manually delete any temporary files if needed."
    exit 0
}
